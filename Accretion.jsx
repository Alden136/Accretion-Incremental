...
/* ============================================================
   ...
   - The pull track is bounded too, but it is priced to run the whole
     ladder: 50 levels, each +0.5% of a second's output, the last one
...
const BALANCE = {
  ...
  tapLevels: 50,      // over a track that spans the ladder, not the first minute
...
/* A pull is worth a share of a second's output — the only scale-free way
   ...
   eight of the ladder's eighty decades. At 36x the last level costs
   1.8e51 kg — about what the last global upgrade costs — so a level lands
   ...
   match — 36 levels at 150x, then 37 at 130x, 40 at 90x, now 50 at 36x —
   which holds both ends still and fits the extra steps in between. To move
   the count again, solve tapBase * growth^(tapLevels-1) back to roughly the
   cost of the last global upgrade.
   tapStep stays at 0.5% a level through all of that, so a longer track is
   also a stronger one: the maximum share has gone 28% to 35%. That is safe
   because it only matters as production when Tidal resonance is pulling for
   you, where it reads as a permanent x1.35 rather than x1.30 — under four
   percent off a run, against a track that is now a quarter longer. */
const tapShare = (s) =>
