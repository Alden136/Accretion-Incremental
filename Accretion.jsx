...
/* ============================================================
   ...
   - The pull track is bounded too, but it is priced to run the whole
     ladder: 60 levels, each +0.5% of a second's output, the last one
...
const BALANCE = {
  ...
  tapLevels: 60,      // over a track that spans the ladder, not the first minute
  tapBase: 1e-25,     // first pull upgrade costs this...
  tapGrowth: 19.5,    // ...and each one after it costs this much more again
  offlineRate: 0.5,
  offlineCapH: 8,
...
/* A pull is worth a share of a second's output — the only scale-free way
   ...
   eight of the ladder's eighty decades. At 19.5x the last level costs
   ...
   match — 36 levels at 150x, 37 at 130x, 40 at 90x, 50 at 36x, now 60 at
   19.5x — which holds both ends still and fits the extra steps in between.
   To move the count again, solve tapBase * growth^(tapLevels-1) back to
   roughly the cost of the last global upgrade.
   tapStep stays at 0.5% a level through all of that, so a longer track is
   also a stronger one: the maximum share has gone 28% to 40%. That is safe
   because it only matters as production when Tidal resonance is pulling for
   you, where it reads as a permanent x1.40 rather than x1.35 — under four
   percent off a run each time, against a track that keeps getting longer.
   The flat bootstrap term doubles per level and reaches 5.8e-9 kg at 60,
   which is still nothing next to production at that depth, and tap resets
   to zero on a collapse, so it can never be carried back to a fresh run. */
const tapShare = (s) =>
